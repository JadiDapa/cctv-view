...
import { createCamera, updateCamera } from "@/lib/networks/camera";
...
export default function UpdateCameraDialog({ camera }: { camera: CameraType }) {
  ...
  const form = useForm<CameraFormType>({
    ...
    values: {
      title: camera.title,
      url: camera.url,
      status: camera.status as "ACTIVE" | "INACTIVE",
    },
  });

  const { mutateAsync, isPending } = useMutation({
    mutationFn: async (values: CameraFormType) => {
      updateCamera(camera.id, values);
    },
    onSuccess: () => {
